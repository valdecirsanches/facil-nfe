...
export function Clients() {
    ...
    if (searchTerm.trim() !== '') {
      ...
      filtered = clients.filter((client) => {
        ...
        return (
          ...
          nomeFantasia.includes(normalizedSearch) ||
          documento.includes(normalizedSearch) ||
          cidade.includes(normalizedSearch) ||
          uf.includes(normalizedSearch)
        )
      })
    }
  ...
  const loadData = async () => {
    ...
    try {
      await db.initialize()
      const [clientsData, transportadorasData] = await Promise.all([
        db.getClients(activeCompanyId),
        db.searchTransportadoras(activeCompanyId, '', 1000),
      ])
  ...
  const handleCEPChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Não buscar automaticamente se o usuário editou manualmente
    if (manualEdit) {
      return
    }
    ...
    try {
      const clientData = {
        ...
        transportadora_id: formData.transportadora_id
          ? parseInt(formData.transportadora_id)
          : null,
      }
      if (editingId) {
  ...
  const handleManualChange = (
    ...
    const { name } = e.target
    // Marcar como edição manual se alterar cidade, UF ou código IBGE
    if (name === 'cidade' || name === 'uf' || name === 'codigo_municipio') {
      setManualEdit(true)
    }
    handleChange(e)
  }
  ...
  return (
    <div className="flex-1 bg-gray-50">
      ...
      <main className="p-8">
        {showAddresses && selectedClientId && activeCompanyId ? (
          <Card className="p-6">
            ...
            <div className="mb-6 flex justify-between items-center gap-4">
              <div className="flex-1 max-w-md relative">
                ...
                <input
                  ...
                  placeholder="Pesquisar por código, nome, documento, cidade..."
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              title="Gerenciar endereços de entrega"
                            >
                              <MapPinIcon size={16} />
                            </button>
                          </div>
                          ...
                          if (
                            ...
                            return (
                              <button
                                ...
                                className={`px-3 py-1 rounded-lg text-sm font-medium transition-colors ${page === currentPage ? 'bg-green-600 text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                              >
                                {page}
                              </button>
                            )
          ...
          <Card className="p-6">
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                />
                <Input
                  label={`${formData.tipo_documento.toUpperCase()} *`}
                  name="documento"
              ...
              {manualEdit && (
                <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-sm text-blue-800">
                    ℹ️ <strong>Modo de edição manual ativado.</strong> Os dados
                    não serão sobrescritos pela busca automática de CEP.
                  </p>
                </div>
              )}
