    ...
    try {
      if (editingId) {
        ...
        if (existingCompany && existingCompany.cnpj !== formData.cnpj) {
          const cnpjExists = companies.some(c => c.cnpj === formData.cnpj && c.id !== editingId);
          if (cnpjExists) {
        ...
        if (cnpjExists) {
          alert('Este CNPJ já está cadastrado. Use um CNPJ diferente ou edite a empresa existente.');
          return;
        }
  ...
  return <div className="flex-1 bg-gray-50">
      ...
      <main className="p-8">
        {!showForm ? <>
            ...
            {companies.length === 0 ? <Card className="p-12 text-center">
                <BuildingIcon size={48} className="mx-auto text-gray-400 mb-4" />
                ...
                <Table headers={['Razão Social', 'CNPJ', 'Cidade/UF', 'Status', 'Ações']}>
                  {companies.map(company => <tr key={company.id} className="hover:bg-gray-50 cursor-pointer" onDoubleClick={() => handleEdit(company)} title="Duplo clique para editar">
                      <td className="px-6 py-4">
                        <div>
                          <p className="font-medium text-gray-900">
                            {company.razao_social}
                          </p>
                          {company.nome_fantasia && <p className="text-sm text-gray-600">
                      ...
                      <td className="px-6 py-4">
                        {activeCompanyId === company.id ? <span className="flex items-center gap-2 text-green-600 text-sm font-medium">
                            <CheckCircleIcon size={16} />
                            Ativa
                          </span> : <span className="text-gray-400 text-sm">Inativa</span>}
                      </td>
                      <td className="px-6 py-4">
                    ...
                    handleEdit(company);
                          ...
                          {activeCompanyId !== company.id && <Button size="sm" variant="secondary" onClick={e => {
                    e.stopPropagation();
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <Input label="CNPJ *" name="cnpj" value={formData.cnpj} onChange={handleChange} placeholder="00.000.000/0000-00" required />
                <Input label="Inscrição Estadual" name="ie" value={formData.ie} onChange={handleChange} />
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Regime Tributário (CRT) *
                  </label>
                  <select name="crt" value={formData.crt} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" required>
                ...
                <Input label="Estado" name="estado" value={formData.estado} onChange={handleChange} placeholder="SP" maxLength={2} readOnly />
