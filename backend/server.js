...
// Listar pedidos
app.get('/api/empresas/:empresaId/pedidos', authenticateToken, async (req, res) => {
  try {
...
app.get('/api/empresas/:empresaId/pedidos/:id', authenticateToken, async (req, res) => {
  try {
...
// Criar novo pedido
app.post('/api/empresas/:empresaId/pedidos', authenticateToken, async (req, res) => {
  try {
    const { empresaId } = req.params
    const { cliente_id, data_entrega, observacoes, itens } = req.body
    ...
    const ultimoPedido = db.prepare('SELECT MAX(id) as max_id FROM pedidos').get()
    const proximoNumero = (ultimoPedido.max_id || 0) + 1
    const numero = `PED-${String(proximoNumero).padStart(6, '0')}`
    ...
    const valor_final = valor_total - desconto_total
    
    // Inserir pedido
    const result = db.prepare(`
      INSERT INTO pedidos (
        numero, cliente_id, data_entrega, valor_total, desconto, valor_final, observacoes
      ) VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(numero, cliente_id, data_entrega, valor_total, desconto_total, valor_final, observacoes)
    ...
    const insertItem = db.prepare(`
      INSERT INTO pedido_itens (
        pedido_id, produto_id, descricao, quantidade, valor_unitario, desconto, valor_total
      ) VALUES (?, ?, ?, ?, ?, ?, ?)
    `)
    
    for (const item of itens) {
      const valor_item = (item.quantidade * item.valor_unitario) - (item.desconto || 0)
      insertItem.run(
        pedidoId,
        item.produto_id,
        item.descricao,
...
// Aprovar pedido
app.put('/api/empresas/:empresaId/pedidos/:id/aprovar', authenticateToken, async (req, res) => {
  try {
    const { empresaId, id } = req.params
    const db = getCompanyDb(empresaId)
...
// Rejeitar pedido
app.put('/api/empresas/:empresaId/pedidos/:id/rejeitar', authenticateToken, async (req, res) => {
  try {
    const { empresaId, id } = req.params
    ...
    db.prepare(`
      ...
      SET status = 'rejeitado', 
          ...
          updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `).run(req.user.id, motivo || 'Sem motivo informado', id)
...
// Cancelar pedido
app.put('/api/empresas/:empresaId/pedidos/:id/cancelar', authenticateToken, async (req, res) => {
  try {
    const { empresaId, id } = req.params
    const db = getCompanyDb(empresaId)
    ...
    if (pedido.status === 'faturado') {
      return res.status(400).json({ error: 'Pedidos faturados não podem ser cancelados' })
    }
...
app.put('/api/empresas/:empresaId/pedidos/:id/vincular-nfe', authenticateToken, async (req, res) => {
  try {
    const { empresaId, id } = req.params
    ...
    res.json({ success: true, message: 'NFe vinculada ao pedido' })
  } catch (error) {
