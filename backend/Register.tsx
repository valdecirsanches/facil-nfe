...
export function Register({
  ...
  const formatCNPJ = (value: string) => {
    ...
    if (numbers.length <= 14) {
      return numbers.replace(/(\d{2})(\d)/, '$1.$2').replace(/(\d{3})(\d)/, '$1.$2').replace(/(\d{3})(\d)/, '$1/$2').replace(/(\d{4})(\d)/, '$1-$2');
    }
    return value;
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (users.length > 0) {
          ...
          // Company exists but no users - allow registration
          setStep('user');
        }
      } else if (response.status === 404) {
        // Company doesn't exist - go to company registration
  ...
  return <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-2xl p-8">
        ...
        {error && !companyHasUsers && <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
        ...
        {companyHasUsers && <div className="mb-6 p-6 bg-amber-50 border-2 border-amber-200 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-amber-800">
                  Para obter acesso, entre em contato com o administrador da
                  empresa e solicite a criação do seu usuário.
                </p>
              </div>
            </div>
          </div>}

        {step === 'cnpj' && !companyHasUsers && <form onSubmit={e => {
        e.preventDefault();
        validateCNPJ();
      }} className="space-y-4">
            <Input label="CNPJ da Empresa" value={cnpj} onChange={handleCNPJChange} placeholder="00.000.000/0000-00" maxLength={18} required disabled={loading} />
        ...
        {step === 'company' && <CompanyRegistration cnpj={cnpj.replace(/\D/g, '')} onSuccess={handleCompanyCreated} onCancel={() => setStep('cnpj')} />}

        {step === 'user' && <form onSubmit={handleUserRegistration} className="space-y-4">
            ...
            <Input label="Nome Completo" value={nome} onChange={e => setNome(e.target.value)} placeholder="Seu nome completo" required disabled={loading} />
            ...
            <Input label="Senha" type="password" value={senha} onChange={e => setSenha(e.target.value)} placeholder="Mínimo 6 caracteres" required disabled={loading} />

            <Input label="Confirmar Senha" type="password" value={confirmarSenha} onChange={e => setConfirmarSenha(e.target.value)} placeholder="Digite a senha novamente" required disabled={loading} />

            <div className="flex gap-3">
              <Button type="button" variant="secondary" onClick={() => setStep('cnpj')} disabled={loading}>
                Voltar
              </Button>
